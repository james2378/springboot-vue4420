<template>
	<div class="diy_details page_video_sharing" id="video_sharing_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_video_sharing'>
							<div_video_sharing :obj="obj"></div_video_sharing>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_video_sharing from "@/components/diy/div_video_sharing.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/video_sharing/set?video_sharing_id=' + obj["video_sharing_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "video_sharing",
							source_field: "video_sharing_id",
							source_id: this.obj.video_sharing_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.video_sharing_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "video_sharing",
					source_field: "video_sharing_id",
					source_id: obj["video_sharing_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/video_sharing/set?video_sharing_id=' + _this.obj["video_sharing_id"], {
								praise_len
							}, (res) => {
								if(res.result){
