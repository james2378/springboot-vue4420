		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/video_sharing/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_video_sharing = !show_list_video_sharing">
		                        <div class="switch_box">
		                            <span v-if="show_list_video_sharing"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--视频分享推荐列表组件 -->
		                <list_video_sharing v-if="show_list_video_sharing" :list="list_video_sharing" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_video_sharing.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_video_sharing" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_video_sharing" :key="i" @click="to_details('video_sharing',o,'video_sharing_id')">
		                            <td v-for="(oj,n) in fields_video_sharing">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_video_sharing from "@/components/diy/list_video_sharing.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_video_sharing: true,
				list_video_sharing: [],
				vm_arr_video_sharing: [
					"video_sharing_id",
					"title",
					"cover",
					"video",
		...
		methods: {
			...
			// 获取视频分享列表
			get_video_sharing() {
				let url = "~/api/video_sharing/get_list?";
				this.$get(url, {
					...
					if (json.result) {
						this.list_video_sharing = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_video_sharing() {
		        var list = this.list_video_sharing;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_video_sharing.map((o) => {
		                // 第二个中括号是对象的属性
